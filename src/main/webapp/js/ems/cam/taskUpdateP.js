...
/*	if(!(mailWorkStatus == "000" || mailWorkStatus == "001" || mailWorkStatus == "201")) {
	...
	if(!(mailWorkStatus == "000" || mailWorkStatus == "001")) {
		alert("발송대기, 발송승인 상태에서만 사용중지 가능합니다.");
		return;
	}
...
/*	if(!(mailWorkStatus == "000" || mailWorkStatus == "001" || mailWorkStatus == "201")) {
	...
	if(!(mailWorkStatus == "000" || mailWorkStatus == "001")) {
		alert("발송대기, 발송승인 상태에서만 삭제 가능합니다.");
		return;
	}
...
function setPopTempContent(tempFlPath) {
	$.getJSON("../tmp/tempFileView.json?tempFlPath=" + tempFlPath, function(res) {
		if(res.result == 'Success') {
			oEditors2.getById["ir1"].exec("SET_CONTENTS", [""]);			//스마트에디터 초기화
			oEditors2.getById["ir1"].exec("PASTE_HTML", [res.tempVal]);	//스마트에디터 내용붙여넣기
		}
	});
}
