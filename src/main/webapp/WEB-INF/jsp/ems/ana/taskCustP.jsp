				...
				<c:if test="${fn:length(custList) > 0}">
					...
					<c:forEach items="${custList}" var="cust" varStatus="custStatus">
						<tr>
							...
							<td>
								<c:out value='${cust.subTaskNo}' />
								차
							</td>
							<td><crypto:decrypt colNm="CUST_ID" data="${cust.custId}" /></td>
							<td><crypto:decrypt colNm="CUST_NM" data="${cust.custNm}" /></td>
