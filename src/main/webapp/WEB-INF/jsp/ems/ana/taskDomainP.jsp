		...
		<table class="grid">
				...
				<c:if test="${fn:length(domainList) > 0}">
					...
					<c:forEach items="${domainList}" var="domain" varStatus="domainStatus">
						<tr>
							...
							<td>
								<fmt:formatNumber var="errorCntNum" type="number" value="${domain.errorCnt}" />
								<c:out value="${errorCntNum}" />
							</td>
							<td>
							...
							<td>
								<fmt:formatNumber var="errorPer" type="percent" value="${(domain.sendCnt - domain.succCnt) == 0 ? 0 : domain.errorCnt / (domain.sendCnt - domain.succCnt)}" />
								<c:out value='${errorPer}' />
							</td>
						</tr>
					</c:forEach>
					<!-- //데이터가 있을 경우 -->
			...
			<tfoot>
				<tr>
					...
					<td>
						<fmt:formatNumber var="sumErrorCntNum" type="number" value="${sumErrorCnt}" />
						<c:out value="${sumErrorCntNum}" />
					</td>
					<td>
						<fmt:formatNumber var="succPer" type="percent" value="${sumSendCnt == 0 ? 0 : sumSuccCnt / sumSendCnt}" />
						<c:out value='${succPer}' />
					</td>
					<td>
						<fmt:formatNumber var="failPer" type="percent" value="${sumSendCnt == 0 ? 0 : sumFailCnt / sumSendCnt}" />
						<c:out value='${failPer}' />
					</td>
					<td>
						<fmt:formatNumber var="errorPer" type="percent" value="${sumFailCnt == 0 ? 0 : sumErrorCnt / sumFailCnt}" />
						<c:out value='${errorPer}' />
					</td>
				</tr>
			</tfoot>
