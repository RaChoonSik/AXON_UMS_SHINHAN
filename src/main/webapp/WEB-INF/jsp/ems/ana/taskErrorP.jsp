...
<div class="graybox">
	<div class="title-area">
		<h3 class="h3-title">
			메일 에러 종류별 건수 <span class="mgl40">총에러건수 : <c:out value='${totCnt}' /></span>
		</h3>
		<!-- btn-wrap// -->
		<div class="btn-wrap">
						...
						<tr>
							<c:if test="${empty tmpNmStep1 || tmpNmStep1 ne error.nmStep1}">
								<td rowspan="<c:out value='${error.step1Rowspan}'/>">
									<c:out value='${error.nmStep1}' />
									:
									<fmt:formatNumber var="cntStep1Num" type="number" value="${error.cntStep1}" />
									<c:out value="${cntStep1Num}" />
								</td>
							</c:if>
							<c:if test="${empty tmpNmStep2 || tmpNmStep2 ne error.nmStep2}">
								<td class="tar" <c:if test="${error.step2Rowspan != 1}"> rowspan="<c:out value='${error.step2Rowspan}'/>"</c:if>>
									<c:out value='${error.nmStep2}' />
									:
									<fmt:formatNumber var="cntStep2Num" type="number" value="${error.cntStep2}" />
									<c:out value="${cntStep2Num}" />
								</td>
							</c:if>
							<c:choose>
								...
								<c:otherwise>
									<td class="tar">
										<c:out value='${error.nmStep3}' />
										:
										<fmt:formatNumber var="cntStep3Num" type="number" value="${error.cntStep3}" />
										<c:out value="${cntStep3Num}" />
									</td>
								</c:otherwise>
							</c:choose>
						</tr>
