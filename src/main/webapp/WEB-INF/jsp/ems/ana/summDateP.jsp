			...
			<tbody>
				...
				<c:if test="${fn:length(dateList) > 0}">
					...
					<c:forEach items="${dateList}" var="date">
						...
						<tr>
							<td>
								<c:out value="${fn:substring(date.ymd,0,4)}-${fn:substring(date.ymd,4,6)}-${fn:substring(date.ymd,6,8)}" />
							</td>
							<td>
								<fmt:formatNumber var="sendCntNum" type="number" value="${date.sendCnt}" />
								<c:out value="${sendCntNum}" />
							</td>
							<td>
								<fmt:formatNumber var="succCntNum" type="number" value="${date.succCnt}" />
								<c:out value="${succCntNum}" />
							</td>
							<td>
								<fmt:formatNumber var="failCntNum" type="number" value="${date.sendCnt - date.succCnt}" />
								<c:out value="${failCntNum}" />
							</td>
							<td>
								<fmt:formatNumber var="openCntNum" type="number" value="${date.openCnt}" />
								<c:out value="${openCntNum}" />
							</td>
							<td>
								<fmt:formatNumber var="validCntNum" type="number" value="${date.validCnt}" />
								<c:out value="${validCntNum}" />
							</td>
							<td>
								<fmt:formatNumber var="clickCntNum" type="number" value="${date.clickCnt}" />
								<c:out value="${clickCntNum}" />
							</td>
							<td>
								<fmt:formatNumber var="blockCntNum" type="number" value="${date.blockCnt}" />
