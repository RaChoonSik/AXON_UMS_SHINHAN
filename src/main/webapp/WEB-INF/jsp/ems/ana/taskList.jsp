				...
				<c:if test="${fn:length(taskList) > 0}">
					...
					<c:forEach items="${taskList}" var="mail" varStatus="mailStatus">
						<tr>
							...
							<td><label><input type="checkbox" name="taskNos" value="<c:out value='${mail.taskNo}'/>"><span></span></label> <input type="checkbox" name="subTaskNos" value="<c:out value='${mail.subTaskNo}'/>" style="display: none;"></td>
							...
							<td><fmt:parseDate var="regDt" value="${mail.regDt}" pattern="yyyyMMddHHmmss" /> <fmt:formatDate var="mailRegDt" value="${regDt}" pattern="yyyy.MM.dd" /> <c:out value='${mailRegDt}' /></td>
							<td><fmt:parseDate var="sendDt" value="${mail.sendDt}" pattern="yyyyMMddHHmm" /> <fmt:formatDate var="mailSendDt" value="${sendDt}" pattern="yyyy.MM.dd HH:mm" /> <c:out value='${mailSendDt}' /></td>
							<td><c:out value='${mail.workStatusNm}' /></td>
							<td><c:out value='${mail.statusNm}' /></td>
							<td><c:if test="${mail.sendRepeat == '001'}">
